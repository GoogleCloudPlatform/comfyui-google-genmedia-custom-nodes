...
class Gemini25FlashImage:
    ...
    def INPUT_TYPES(cls) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "required": {
                "model": (
                    [GeminiFlashImageModel.GEMINI_25_FLASH_IMAGE.name],
...
class Gemini31FlashImage:
    """
    A ComfyUI node for generating images from text prompts using the Google Gemini Flash Image API.
    """
    ...
    def generate_and_return_image(
        ...
        if aspect_ratio not in GEMINI_31_FLASH_IMAGE_ASPECT_RATIO:
            raise RuntimeError(
                f"Invalid aspect ratio: {aspect_ratio}. Valid aspect ratios are: {GEMINI_31_FLASH_IMAGE_ASPECT_RATIO}."
            )
