...
from .constants import (
    ...
    IMAGE_MIME_TYPES,
    VIDEO_MIME_TYPES,
    GeminiModel, Gemini31Model, Gemini35Model,
    ThresholdOptions,
)
...
class GeminiNode31(VertexAIClient):
    def __init__(
        self, gcp_project_id: Optional[str] = None, gcp_region: Optional[str] = None
    ):
        """
    ...
    def INPUT_TYPES(s):
        ...
        return {
            "required": {
                ...
                "model": (
                    [model.name for model in Gemini31Model],
                    {"default": Gemini31Model.GEMINI_31_PRO.name},
                ),
                # GenerationConfig Parameters
                "temperature": (
                    "FLOAT",
...
class GeminiNode35(VertexAIClient):
    def __init__(
        self, gcp_project_id: Optional[str] = None, gcp_region: Optional[str] = None
    ):
        """
    ...
    def generate_content(
        ...
        try:
            ...
            # Make the API call
            logger.info(
                f"Making Gemini API call with the following Model : {Gemini35Model[model]} , config {gen_config_obj}"
            )  # Prepare Safety Settings
